	...
	if (typeof module === "object" && typeof module.exports === "object") {
		...
		(
				...
				function(window, noGlobal) {
					...
					var support = {};
					var document = window.document, version = "2.1.1", jQuery = function(
							selector, context) {
						return new jQuery.fn.init(selector, context)
					}, rtrim = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, rmsPrefix = /^-ms-/, rdashAlpha = /-([\da-z])/gi, fcamelCase = function(
							all, letter) {
					...
					jQuery.fn = jQuery.prototype = {
						...
						get : function(num) {
							return num != null ? num < 0 ? this[num
									+ this.length] : this[num] : slice
									.call(this)
						},
					...
					var Sizzle = function(window) {
						var i, support, Expr, getText, isXML, tokenize, compile, select, outermostContext, sortInput, hasDuplicate, setDocument, document, docElem, documentIsHTML, rbuggyQSA, rbuggyMatches, matches, contains, expando = "sizzle"
								+ -new Date, preferredDoc = window.document, dirruns = 0, done = 0, classCache = createCache(), tokenCache = createCache(), compilerCache = createCache(), sortOrder = function(
								a, b) {
							if (a === b) {
								hasDuplicate = true
							}
							return 0
						}, strundefined = typeof undefined, MAX_NEGATIVE = 1 << 31, hasOwn = {}.hasOwnProperty, arr = [], pop = arr.pop, push_native = arr.push, push = arr.push, slice = arr.slice, indexOf = arr.indexOf
								|| function(elem) {
									...
									return -1
								}, booleans = "checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|readonly|required|scoped", whitespace = "[\\x20\\t\\r\\n\\f]", characterEncoding = "(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+", identifier = characterEncoding
								.replace("w", "w#"), attributes = "\\["
								+ whitespace
								+ "*("
								+ characterEncoding
								+ ")(?:"
								...
								"^" + whitespace
										+ "+|((?:^|[^\\\\])(?:\\\\.)*)"
										+ whitespace + "+$", "g"), rcomma = new RegExp(
								"^" + whitespace + "*," + whitespace + "*"), rcombinators = new RegExp(
								"^" + whitespace + "*([>+~]|" + whitespace
										+ ")" + whitespace + "*"), rattributeQuotes = new RegExp(
								"=" + whitespace + "*([^\\]'\"]*?)"
										+ whitespace + "*\\]", "g"), rpseudo = new RegExp(
								pseudos), ridentifier = new RegExp("^"
							...
							needsContext : new RegExp(
									"^"
											...
											+ whitespace + "*\\)|)(?=[^-]|$)",
									"i")
						}, rinputs = /^(?:input|select|textarea|button)$/i, rheader = /^h\d$/i, rnative = /^[^{]+\{\s*\[native \w/, rquickExpr = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/, rsibling = /[+~]/, rescape = /'|\\/g, runescape = new RegExp(
								"\\\\([\\da-f]{1,6}" + whitespace + "?|("
						...
						Sizzle.attr = function(elem, name) {
							...
							var fn = Expr.attrHandle[name.toLowerCase()], val = fn
						...
						Expr = Sizzle.selectors = {
							...
							filter : {
								...
								CHILD : function(type, what, argument, first,
									...
									return first === 1 && last === 0 ? function(
											elem) {
						...
						tokenize = Sizzle.tokenize = function(selector,
							...
							return parseOnly ? soFar.length : soFar ? Sizzle
									.error(selector) : tokenCache(selector,
									groups).slice(0)
						};
						...
						function setMatcher(preFilter, selector, matcher,
							...
							return markFunction(function(seed, results,
									context, xml) {
								var temp, i, elem, preMap = [], postMap = [], preexisting = results.length, elems = seed
										|| multipleContexts(selector || "*",
												context.nodeType ? [ context ]
														: context, []), matcherIn = preFilter
										&& (seed || !selector) ? condense(
										elems, preMap, preFilter, context, xml)
										: elems, matcherOut = matcher ? postFinder
						...
						function matcherFromTokens(tokens) {
							var checkContext, matcher, j, len = tokens.length, leadingRelative = Expr.relative[tokens[0].type], implicitRelative = leadingRelative
									...
									: 0, matchContext = addCombinator(function(
									elem) {
						...
						function matcherFromGroupMatchers(elementMatchers,
							...
							var bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0, superMatcher = function(
								...
								var elem, j, matcher, matchedCount = 0, i = "0", unmatched = seed
										&& [], setMatched = [], contextBackup = outermostContext, elems = seed
										|| byElement
										&& Expr.find["TAG"]("*", outermost), dirrunsUnique = dirruns += contextBackup == null ? 1
										: Math.random() || .1, len = elems.length;
								if (outermost) {
					...
					var rootjQuery, rquickExpr = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/, init = jQuery.fn.init = function(
							selector, context) {
						var match, elem;
						if (!selector) {
							return this
							...
							.extend({
								...
								when : function(subordinate) {
									var i = 0, resolveValues = slice
											.call(arguments), length = resolveValues.length, remaining = length !== 1
											|| subordinate
											&& jQuery
													.isFunction(subordinate.promise) ? length
											: 0, deferred = remaining === 1 ? subordinate
											: jQuery.Deferred(), updateFunc = function(
											i, contexts, values) {
					...
					function actualDisplay(name, doc) {
						var style, elem = jQuery(doc.createElement(name))
								.appendTo(doc.body), display = window.getDefaultComputedStyle
								&& (style = window
										.getDefaultComputedStyle(elem[0])) ? style.display
												...
												reliableMarginRight : function() {
													var ret, marginDiv = div
															.appendChild(document
																	.createElement("div"));
													marginDiv.style.cssText = div.style.cssText = "-webkit-box-sizing:content-box;-moz-box-sizing:content-box;"
					...
					var rdisplayswap = /^(none|table(?!-c[ea]).+)/, rnumsplit = new RegExp(
					...
					function setPositiveNumber(elem, value, subtract) {
						...
						return matches ? Math.max(0, matches[1]
								- (subtract || 0))
								+ (matches[2] || "px") : value
					}
					...
					var fxNow, timerId, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = new RegExp(
						...
						"*" : [ function(prop, value) {
							var tween = this.createTween(prop, value), target = tween
									...
									&& parts[3]
									|| (jQuery.cssNumber[prop] ? "" : "px"), start = (jQuery.cssNumber[prop] || unit !== "px"
									&& +target)
									&& rfxnum
						...
						var result, stopped, index = 0, length = animationPrefilters.length, deferred = jQuery
							...
							var currentTime = fxNow || createFxNow(), remaining = Math
									.max(0, animation.startTime
											+ animation.duration - currentTime), temp = remaining
							...
							if (percent < 1 && length) {
								...
								.promise({
									...
									stop : function(gotoEnd) {
										var index = 0, length = gotoEnd ? animation.tweens.length
												: 0;
										if (stopped) {
											return this
										}
					...
					var ajaxLocParts, ajaxLocation, rhash = /#.*$/, rts = /([?&])_=[^&]*/, rheaders = /^(.*?):[ \t]*([^\r\n]*)$/gm, rlocalProtocol = /^(?:about|app|app-storage|.+-extension|file|res|widget):$/, rnoContent = /^(?:GET|HEAD)$/, rprotocol = /^\/\//, rurl = /^([\w.+-]+:)(?:\/\/(?:[^\/?#]*@|)([^\/?#:]*)(?::(\d+)|)|)/, prefilters = {}, transports = {}, allTypes = "*/"
							.concat("*");
					try {
					...
					function ajaxConvert(s, response, jqXHR, isSuccess) {
						var conv2, current, conv, tmp, prev, converters = {}, dataTypes = s.dataTypes
								.slice();
						if (dataTypes[1]) {
					...
					jQuery
							.extend({
								...
								ajax : function(url, options) {
									...
									options = options || {};
									var transport, cacheURL, responseHeadersString, responseHeaders, timeoutTimer, parts, fireGlobals, i, s = jQuery
											.ajaxSetup({}, options), callbackContext = s.context
											|| s, globalEventContext = s.context
											&& (callbackContext.nodeType || callbackContext.jquery) ? jQuery(callbackContext)
											: jQuery.event, deferred = jQuery
											.Deferred(), completeDeferred = jQuery
											.Callbacks("once memory"), statusCode = s.statusCode
											|| {}, requestHeaders = {}, requestHeadersNames = {}, state = 0, strAbort = "canceled", jqXHR = {
										readyState : 0,
									...
									if (!s.hasContent) {
										...
										if (s.cache === false) {
											s.url = rts.test(cacheURL) ? cacheURL
													.replace(rts, "$1_="
															+ nonce++)
													: cacheURL
															+ (rquery
	...
	if (typeof define === "function" && define.amd) {
		...
		(function($) {
			...
			$.fn
					.extend({
						scrollParent : function() {
							var position = this.css("position"), excludeStaticParent = position === "absolute", scrollParent = this
									.parents()
									.filter(
											function() {
												var parent = $(this);
			...
			$.widget = function(name, base, prototype) {
				var fullName, existingConstructor, constructor, basePrototype, proxiedPrototype = {}, namespace = name
						.split(".")[0];
				name = name.split(".")[1];
			...
			$.Widget.prototype = {
				...
				_createWidget : function(options, element) {
					...
					if (element !== this) {
						...
						this.document = $(element.style ? element.ownerDocument
				...
				$.position = {
					...
					getScrollInfo : function(within) {
						var overflowX = within.isWindow || within.isDocument ? ""
								...
								|| overflowX === "auto"
								&& within.width < within.element[0].scrollWidth, hasOverflowY = overflowY === "scroll"
								|| overflowY === "auto"
					...
					.widget(
							...
							$.ui.mouse,
							{
								...
								_isRootNode : function(element) {
									return /(html|body)/i.test(element.tagName)
							...
							"scroll",
							{
								start : function(event, ui, i) {
									if (i.scrollParent[0] !== i.document[0]
										...
										i.overflowOffset = i.scrollParent
												.offset()
									}
								},
			...
			$.ui.intersect = function() {
				...
				return function(draggable, droppable, toleranceMode) {
					...
					case "pointer":
						...
						return isOverAxis(draggableTop, t, droppable
								.proportions().height)
								&& isOverAxis(draggableLeft, l, droppable
										.proportions().width);
					case "touch":
						return (y1 >= t && y1 <= b || y2 >= t && y2 <= b || y1 < t
								&& y2 > b)
								&& (x1 >= l && x1 <= r || x2 >= l && x2 <= r || x1 < l
										&& x2 > r);
					default:
						return false
					}
			...
			$.ui.ddmanager = {
				...
				prepareOffsets : function(t, event) {
					var i, j, m = $.ui.ddmanager.droppables[t.options.scope]
							|| [], type = event ? event.type : null, list = (t.currentItem || t.element)
					...
					.widget(
							...
							$.ui.mouse,
							{
								...
								_mouseDrag : function(event) {
									var data, el = this.helper, props = {}, smp = this.originalMousePosition, a = this.axis, dx = event.pageX
											- smp.left || 0, dy = event.pageY
											- smp.top || 0, trigger = this._change[a];
									this.prevPosition = {
								...
								stop : function(event) {
									var that = $(this).resizable("instance"), o = that.options, pr = that._proportionallyResizeElements, ista = pr.length
										...
										height : that.size.height - soffseth
											...
											that.element.css("left"), 10)
											+ (that.position.left - that.originalPosition.left)
											|| null, top = parseInt(
											that.element.css("top"), 10)
											+ (that.position.top - that.originalPosition.top)
											|| null;
								...
								resize : function(event, ui) {
									...
									if (cp.left < (that._helper ? co.left : 0)) {
										that.size.width = that.size.width
												+ (that._helper ? that.position.left
														- co.left
														: that.position.left
																- cop.left);
										if (pRatio) {
									...
									if (cp.top < (that._helper ? co.top : 0)) {
										that.size.height = that.size.height
												+ (that._helper ? that.position.top
														- co.top
														: that.position.top);
										if (pRatio) {
							...
							"alsoResize",
							{
								...
								resize : function(event, ui) {
									var that = $(this).resizable("instance"), o = that.options, os = that.originalSize, op = that.originalPosition, delta = {
										height : that.size.height - os.height
												|| 0,
										width : that.size.width - os.width || 0,
										top : that.position.top - op.top || 0,
										left : that.position.left - op.left
												|| 0
							...
							$.ui.mouse,
							{
								...
								_intersectsWith : function(item) {
									var x1 = this.positionAbs.left, x2 = x1
											...
											|| y1 + dyClick > t
											&& y1 + dyClick < b, isOverElementWidth = this.options.axis === "y"
											|| x1 + dxClick > l
											&& x1 + dxClick < r, isOverElement = isOverElementHeight
											&& isOverElementWidth;
									if (this.options.tolerance === "pointer"
					...
					.widget(
							"ui.autocomplete",
							{
								...
								_create : function() {
									...
									this.isMultiLine = isTextarea ? true
											: isInput ? false : this.element
								...
								_resizeMenu : function() {
									...
									ul.outerWidth(Math.max(ul.width("")
							...
							Datepicker.prototype,
							{
								...
								_checkOffset : function(inst, offset, isFixed) {
									...
									offset.left -= this._get(inst, "isRTL") ? dpWidth
											...
											: 0;
									offset.left -= isFixed
											&& offset.left === inst.input
													.offset().left ? $(document)
											.scrollLeft()
											: 0;
									offset.top -= isFixed
											&& offset.top === inst.input
													.offset().top
									...
									offset.left -= Math
											.min(
													offset.left,
													offset.left + dpWidth > viewWidth
															&& viewWidth > dpWidth ? Math
															.abs(offset.left
																	+ dpWidth
																	- viewWidth)
															: 0);
									offset.top -= Math
											.min(
													offset.top,
													offset.top + dpHeight > viewHeight
															&& viewHeight > dpHeight ? Math
															.abs(dpHeight
																	+ inputHeight)
															: 0);
									return offset
								...
								iso8601Week : function(date) {
									...
									return Math
											.floor(Math
													.round((time - checkDate) / 864e5) / 7) + 1
								},
								parseDate : function(format, value, settings) {
									...
									var iFormat, dim, extra, iValue = 0, shortYearCutoffTemp = (settings ? settings.shortYearCutoff
											...
											: null)
											|| this._defaults.monthNames, year = -1, month = -1, day = -1, doy = -1, literal = false, date, lookAhead = function(
											match) {
								...
								formatDate : function(format, date, settings) {
									...
									var iFormat, dayNamesShort = (settings ? settings.dayNamesShort
											...
											|| this._defaults.monthNamesShort, monthNames = (settings ? settings.monthNames
											: null)
											|| this._defaults.monthNames, lookAhead = function(
											match) {
										var matches = iFormat + 1 < format.length
												&& format.charAt(iFormat + 1) === match;
										if (matches) {
										...
										return matches
									}, formatNumber = function(match, value,
											len) {
										var num = "" + value;
										if (lookAhead(match)) {
								...
								_possibleChars : function(format) {
									var iFormat, chars = "", literal = false, lookAhead = function(
											match) {
										var matches = iFormat + 1 < format.length
												&& format.charAt(iFormat + 1) === match;
										if (matches) {
								...
								_generateHTML : function(inst) {
									var maxDraw, prevText, prev, nextText, next, currentText, gotoDate, controls, buttonPanel, firstDay, showWeek, dayNames, dayNamesMin, monthNames, monthNamesShort, beforeShowDay, showOtherMonths, selectOtherMonths, defaultDate, html, dow, row, group, col, selectedDate, cornerClass, calender, thead, day, daysInMonth, leadDays, curRows, numRows, printDate, dRow, tbody, daySettings, otherMonth, unselectable, tempDate = new Date, today = this
											._daylightSavingAdjust(new Date(
													...
													tempDate.getMonth(),
													tempDate.getDate())), isRTL = this
											._get(inst, "isRTL"), showButtonPanel = this
											...
											._get(inst, "stepMonths"), isMultiMonth = numMonths[0] !== 1
											|| numMonths[1] !== 1, currentDate = this
								...
								_generateMonthYearHeader : function(inst,
									...
									if (secondary || !changeMonth) {
										...
										inMinYear = minDate
												&& minDate.getFullYear() === drawYear;
										inMaxYear = maxDate
												&& maxDate.getFullYear() === drawYear;
			...
			function datepicker_bindHover(dpDiv) {
				...
				return dpDiv
						...
						.delegate(
								...
								function() {
									if (!$.datepicker
											._isDisabledDatepicker(datepicker_instActive.inline ? dpDiv
													.parent()[0]
					...
					.widget(
							"ui.dialog",
							{
								...
								_makeResizable : function() {
									var that = this, options = this.options, handles = options.resizable, position = this.uiDialog
											.css("position"), resizeHandles = typeof handles === "string" ? handles
											: "n,e,s,w,se,sw,ne,nw";
									function filteredUi(ui) {
										return {
								...
								_createRange : function() {
									...
									if (options.range) {
										...
										this.range
												.addClass(classes
														+ (options.range === "min"
																|| options.range === "max" ? " ui-slider-range-"
																+ options.range
																: ""))
									} else {
										if (this.range) {
			...
			(function(jQuery, undefined) {
				var stepHooks = "backgroundColor borderBottomColor borderLeftColor borderRightColor borderTopColor color columnRuleColor outlineColor textDecorationColor textEmphasisColor", rplusequals = /^([\-+])=\s*(\d+\.?\d*)/, stringParsers = [
						...
						} ], color = jQuery.Color = function(color, green,
						blue, alpha) {
				...
				spaces.hsla.from = function(hsla) {
					...
					var h = hsla[0] / 360, s = hsla[1], l = hsla[2], a = hsla[3], q = l <= .5 ? l
							* (1 + s)
							: l + s - l * s, p = 2 * l - q;
				...
				each(
						...
						function(spaceName, space) {
							...
							color.fn[spaceName] = function(value) {
								...
								var ret, type = jQuery.type(value), arr = type === "array"
										|| type === "object" ? value
			...
			(function() {
				...
				$.each(baseEasings, function(name, easeIn) {
					...
					$.easing["easeInOut" + name] = function(p) {
						return p < .5 ? easeIn(p * 2) / 2 : 1 - easeIn(p * -2
								+ 2) / 2
					}
				})
			})();
			...
			var effectBounce = $.effects.effect.bounce = function(o, done) {
				var el = $(this), props = [ "position", "top", "bottom",
						...
						.setMode(el, o.mode || "effect"), hide = mode === "hide", show = mode === "show", direction = o.direction
						|| "up", distance = o.distance, times = o.times || 5, anims = times
						* 2 + (show || hide ? 1 : 0), speed = o.duration
						/ anims, easing = o.easing, ref = direction === "up"
						|| direction === "down" ? "top" : "left", motion = direction === "up"
						|| direction === "left", i, upAnim, downAnim, queue = el
						.queue(), queuelen = queue.length;
				if (show || hide) {
					props.push("opacity")
				}
				$.effects.save(el, props);
			...
			var effectDrop = $.effects.effect.drop = function(o, done) {
				var el = $(this), props = [ "position", "top", "bottom",
						...
						.setMode(el, o.mode || "hide"), show = mode === "show", direction = o.direction
						|| "left", ref = direction === "up"
						|| direction === "down" ? "top" : "left", motion = direction === "up"
						|| direction === "left" ? "pos" : "neg", animation = {
					opacity : show ? 1 : 0
				}, distance;
				$.effects.save(el, props);
				el.show();
			...
			var effectExplode = $.effects.effect.explode = function(o, done) {
				var rows = o.pieces ? Math.round(Math.sqrt(o.pieces)) : 3, cells = rows, el = $(this), mode = $.effects
						.setMode(el, o.mode || "hide"), show = mode === "show", offset = el
						.show().css("visibility", "hidden").offset(), width = Math
						.ceil(el.outerWidth() / cells), height = Math.ceil(el
						.outerHeight()
						/ rows), pieces = [], i, j, left, top, mx, my;
			...
			var effectSize = $.effects.effect.size = function(o, done) {
				var original, baseline, factor, el = $(this), props0 = [
						...
						"height", "overflow", "opacity" ], props1 = [
						"position", "top", "bottom", "left", "right",
						"overflow", "opacity" ], props2 = [ "width", "height",
						"overflow" ], cProps = [ "fontSize" ], vProps = [
						"borderTopWidth", "borderBottomWidth", "paddingTop",
						"paddingBottom" ], hProps = [ "borderLeftWidth",
						"borderRightWidth", "paddingLeft", "paddingRight" ], mode = $.effects
						.setMode(el, o.mode || "effect"), restore = o.restore
			...
			var effectPulsate = $.effects.effect.pulsate = function(o, done) {
				var elem = $(this), mode = $.effects.setMode(elem, o.mode
						|| "show"), show = mode === "show", hide = mode === "hide", showhide = show
						|| mode === "hide", anims = (o.times || 5) * 2
						+ (showhide ? 1 : 0), duration = o.duration / anims, animateTo = 0, queue = elem
			...
			var effectShake = $.effects.effect.shake = function(o, done) {
				var el = $(this), props = [ "position", "top", "bottom",
						...
						.round(o.duration / anims), ref = direction === "up"
						|| direction === "down" ? "top" : "left", positiveMotion = direction === "up"
						|| direction === "left", animation = {}, animation1 = {}, animation2 = {}, i, queue = el
						.queue(), queuelen = queue.length;
				$.effects.save(el, props);
				el.show();
				$.effects.createWrapper(el);
			...
			var effectSlide = $.effects.effect.slide = function(o, done) {
				var el = $(this), props = [ "position", "top", "bottom",
						...
						|| "left", ref = direction === "up"
						|| direction === "down" ? "top" : "left", positiveMotion = direction === "up"
						|| direction === "left", distance, animation = {};
				$.effects.save(el, props);
				el.show();
				distance = o.distance
						|| el[ref === "top" ? "outerHeight" : "outerWidth"]
...
(function(window) {
	...
	if (!window.document.addEventListener) {
		window.document.attachEvent("DOMContentLoaded",
				DOMContentLoadedListener)
	} else {
		window.document.addEventListener("DOMContentLoaded",
				DOMContentLoadedListener, false)
	}
	return;
		...
		(function($, window, document, undefined) {
					...
					.widget(
							"selectBox.selectBoxIt",
							{
								...
								_create : function(internal) {
									var self = this, populateOption = self.options["populate"];
									...
									self.theme = self.getThemes()[self.options["theme"]]
								...
								_createUnorderedList : function() {
									var self = this, dataDisabled, optgroupClass, optgroupElement, iconClass, iconUrl, iconUrlClass, iconUrlStyle, currentItem = "", originalElemId = self.originalElemId
											|| "", createdList = $("<ul/>", {
										id : originalElemId && originalElemId
												+ "SelectBoxItOptions",
										"class" : "selectboxit-options",
										tabindex : -1
									}), currentDataSelectedText, currentDataText, currentText, parent;
								...
								_replaceSelectBox : function() {
									var self = this, height, originalElemId = self.originalElem.id
											|| "", size = self.selectBox
									...
									if (!self
											._realOuterWidth(self.dropdownImageContainer)) {
										...
										if (self.dropdown.is(":visible")) {
											...
											self.dropdown
													...
													.css(
															{
																width : self
																		._realOuterWidth(self.list)
																		+ self
																				._realOuterWidth(self.downArrowContainer)
																		+ self
																				._realOuterWidth(self.dropdownImage)
															})
										}
										self.list.css({
									...
									self.dropdownText
											.css({
												"max-width" : self.dropdownContainer
														.width()
														- (self.downArrowContainer
																.outerWidth(true) + self.dropdownImage
																.outerWidth(true))
											});
								...
								_scrollToView : function(type) {
									var self = this, currentOption = self.listItems
											.eq(self.currentFocus), listScrollTop = self.list
											.scrollTop(), currentItemHeight = currentOption
											.height(), currentTopPosition = currentOption
											.position().top, absCurrentTopPosition = Math
											.abs(currentTopPosition), listHeight = self.list
											.height(), currentText;
									if (type === "search") {
									...
									this.dropdown
											.on({
												...
												"keydown.selectBoxIt" : function(
														e) {
													var currentKey = self._keyMappings[e.keyCode], keydownMethod = self
												...
												"keypress.selectBoxIt" : function(
													...
													var currentKey = e.charCode
															|| e.keyCode, key = self._keyMappings[e.charCode
															|| e.keyCode], alphaNumericKey = String
											...
											.on(
													{
														...
														"focusin.selectBoxIt" : function() {
															...
															var listIsHidden = self.list
																	.is(":hidden");
															if (self.options["searchWhenHidden"]
																	&& listIsHidden
												...
												"open.selectBoxIt" : function() {
													...
													self.listItems
															...
															.add(
																	self.listAnchors
																			.not(activeElem
																					.find(".selectboxit-option-anchor")))
															.removeClass(
																	focusClass);
													activeElem
															.addClass(
								...
								triggerEvent : function(eventName) {
									var self = this, currentIndex = self.options["showFirstOption"] ? self.currentFocus
			...
			selectBoxIt._ariaAccessibility = function() {
						...
						.attr(
								{
									...
									"aria-activedescendant" : self.listItems
											.eq(self.currentFocus).length ? self.listItems
											.eq(self.currentFocus)[0].id
											: "",
									"aria-label" : $(
											"label[for='"
				...
				self.selectBox.on({
					"change.selectBoxIt" : function() {
						self.dropdownText.attr("aria-label",
								self.originalElem.value)
					},
					"open.selectBoxIt" : function() {
			...
			selectBoxIt.disable = function(callback) {
				...
				if (!self.options["disabled"]) {
					...
					self.dropdown.removeAttr("tabindex").removeClass(
							self.theme["enabled"]).addClass(
							self.theme["disabled"]);
			...
			selectBoxIt._dynamicPositioning = function() {
				...
				if ($.type(self.listSize) === "number") {
					...
					var listOffsetTop = self.dropdown.offset().top, listHeight = self.list
							.data("max-height")
							|| self.list.outerHeight(), selectBoxHeight = self.dropdown
							.outerHeight(), viewport = self.options["viewport"], viewportHeight = viewport
							.height(), viewportScrollTop = $.isWindow(viewport
							.get(0)) ? viewport.scrollTop()
							: viewport.offset().top, topToBottom = listOffsetTop
							+ selectBoxHeight + listHeight <= viewportHeight
							+ viewportScrollTop, bottomReached = !topToBottom;
					...
					if (!bottomReached) {
						...
						self.list.css("max-height", listHeight);
						self.list.css("top", self.dropdown.position().top
								- self.list.outerHeight())
					} else {
						var outsideBottomViewport = Math.abs(listOffsetTop
								+ selectBoxHeight + listHeight
								- (viewportHeight + viewportScrollTop)), outsideTopViewport = Math
								.abs(self.dropdown.offset().top
										- viewportScrollTop - listHeight);
						if (outsideBottomViewport < outsideTopViewport) {
							self.list.css("max-height", listHeight
									- outsideBottomViewport - selectBoxHeight
									/ 2);
							self.list.css("top", "auto")
						} else {
							self.list.css("max-height", listHeight
			...
			selectBoxIt.enable = function(callback) {
				...
				if (self.options["disabled"]) {
					...
					self.dropdown.attr("tabindex", 0).removeClass(
							self.theme["disabled"]).addClass(
							self.theme["enabled"]);
			...
			selectBoxIt.moveDown = function(callback) {
				...
				var disabled = self.listItems.eq(self.currentFocus).attr(
						"data-disabled") === "true" ? true : false, hasNextEnabled = self.listItems
						.eq(self.currentFocus).nextAll("li").not(
								"[data-disabled='true']").first().length;
				if (self.currentFocus === self.listItems.length) {
					...
					self.listItems.eq(self.currentFocus - 1).blur().end().eq(
							self.currentFocus).focusin();
			...
			selectBoxIt.moveUp = function(callback) {
				...
				self.currentFocus -= 1;
				var disabled = self.listItems.eq(self.currentFocus).attr(
						"data-disabled") === "true" ? true : false, hasPreviousEnabled = self.listItems
						.eq(self.currentFocus).prevAll("li").not(
								"[data-disabled='true']").first().length;
				if (self.currentFocus === -1) {
					...
					self.listItems.eq(this.currentFocus + 1).blur().end().eq(
							self.currentFocus).focusin();
				...
				for (x = currentIndex, arrayLength = textArray.length; x < arrayLength; x += 1) {
					...
					if (!matchExists) {
						self.currentText = self.currentText.charAt(
								self.currentText.length - 1).replace(
								/[|()\[{.+*?$\\]/g, "\\$0");
						...
						.on(
								{
									"changed.selectBoxIt" : function() {
										currentOption = self.selectBox.find(
...
var Froogaloop = function() {
	...
	Froogaloop.fn = Froogaloop.prototype = {
		...
		api : function(method, valueOrCallback) {
			...
			var self = this, element = self.element, target_id = element.id !== "" ? element.id
					: null, params = !isFunction(valueOrCallback) ? valueOrCallback
		...
		removeEvent : function(eventName) {
			...
			var self = this, element = self.element, target_id = element.id !== "" ? element.id
					: null, removed = removeCallback(eventName, target_id);
	...
	function postMessage(method, params, target) {
		...
		var url = target.getAttribute("src").split("?")[0], data = JSON
				.stringify({
					method : method,
					value : params
				});
	...
	function onMessageReceived(event) {
		...
		var value = data.value, eventData = data.data, target_id = target_id === "" ? null
				: data.player_id, callback = getCallback(method, target_id), params = [];
		if (!callback) {
			return false
		}
		if (value !== undefined) {
		...
		return params.length > 0 ? callback.apply(null, params) : callback
				.call()
	}
					...
					function($, window) {
						...
						Context = function() {
							function Context($element) {
								...
								$element.bind(scrollEvent, function() {
									...
									if (!(_this.didScroll || isTouch)) {
										...
										return window.setTimeout(scrollHandler,
								...
								$element.bind(resizeEvent, function() {
									...
									if (!_this.didResize) {
										...
										return window.setTimeout(resizeHandler,
								...
								$
										.each(
												...
												function(aKey, axis) {
													...
													direction = isForward ? axis.forward
															: axis.backward;
													$
															.each(
																	...
																	function(
																			...
																			waypoint) {
																		var _ref, _ref1;
																		if (axis.oldScroll < (_ref = waypoint.offset)
																				&& _ref <= axis.newScroll) {
																			return triggered
																					.push(waypoint)
																		} else if (axis.newScroll < (_ref1 = waypoint.offset)
																				&& _ref1 <= axis.oldScroll) {
																			return triggered
																					.push(waypoint)
																		}
																	});
													triggered.sort(function(a,
															b) {
														return a.offset
																- b.offset
													});
													...
													return $
															.each(
																	...
																	function(i,
																			waypoint) {
																		if (waypoint.options.continuous
													...
													return $
															.each(
																	...
																	function(i,
																		...
																		elementOffset = $
																				.isWindow(waypoint.element) ? 0
																				: waypoint.$element
																						.offset()[axis.offsetProp];
																		if ($
																				.isFunction(adjustment)) {
																			adjustment = adjustment
																					.apply(waypoint.element)
																		} else if (typeof adjustment === "string") {
																			adjustment = parseFloat(adjustment);
																			if (waypoint.options.offset
																					.indexOf("%") > -1) {
																				adjustment = Math
																						.ceil(axis.contextDimension
																								* adjustment
																								/ 100)
																			}
																		}
							...
							Context.prototype.checkEmpty = function() {
								if ($.isEmptyObject(this.waypoints.horizontal)
										&& $
												.isEmptyObject(this.waypoints.vertical)) {
									this.$element.unbind([ resizeEvent,
											scrollEvent ].join(" "));
						...
						Waypoint = function() {
							...
							Waypoint.getWaypointsByElement = function(element) {
								...
								all = $.extend({}, allWaypoints.horizontal,
						...
						methods = {
							init : function(f, options) {
								...
								this
										.each(function() {
											...
											$this = $(this);
											contextElement = (_ref1 = options.context) != null ? _ref1
													: $.fn[wp].defaults.context;
											if (!$.isWindow(contextElement)) {
												contextElement = $this
														.closest(contextElement)
											}
											...
											return new Waypoint($this, context,
													options)
										});
								$[wps]("refresh");
								return this
							...
							_invoke : function(method) {
								this.each(function() {
									var waypoints;
									waypoints = Waypoint
											.getWaypointsByElement(this);
									return $.each(waypoints, function(i,
											waypoint) {
										waypoint[method]();
										return true
									})
						...
						$.fn[wp] = function() {
							var args, method;
							method = arguments[0],
									args = 2 <= arguments.length ? __slice
											.call(arguments, 1) : [];
							if (methods[method]) {
						...
						jQMethods = {
							...
							viewportHeight : function() {
								var _ref;
								return (_ref = window.innerHeight) != null ? _ref
										: $w.height()
							},
							aggregate : function(contextSelector) {
								...
								if (contextSelector) {
									collection = (_ref = contexts[$(contextSelector)[0][contextKey]]) != null ? _ref.waypoints
											: void 0
								}
								if (!collection) {
									return []
								...
								$.each(waypoints, function(axis, arr) {
									$.each(collection[axis], function(key,
											waypoint) {
									...
									return waypoints[axis] = $
											.unique(waypoints[axis])
								});
								return waypoints
							},
							...
							_invoke : function(method) {
								var waypoints;
								waypoints = $.extend({}, allWaypoints.vertical,
										allWaypoints.horizontal);
								return $.each(waypoints,
										function(key, waypoint) {
											waypoint[method]();
											return true
										})
							},
							_filter : function(selector, axis, test) {
								...
								waypoints = [];
								$.each(context.waypoints[axis], function(i,
										waypoint) {
...
$(function() {
	...
	$(document).on(
			...
			function(e) {
				...
				data.push({
					name : "city",
					value : $("select[name=market]").val()
				});
				button.attr("disabled", "disabled").addClass("busy")
						.removeClass("submit");
				$.post(form.attr("action"), data, function(data) {
					if (data.success && data.url) {
						...
						$(".error", form).hide();
						$(".success", form).html(data.message).show()
					} else {
						$(".success", form).hide();
						$(".error", form).html(data.error).show()
					}
					form.find("input[name=emails]").val("");
					button.removeAttr("disabled").removeClass("busy").addClass(
							"submit")
				});
				e.preventDefault()
			});
	$("select[name=market]").change(function() {
		$.cookie("website_market_slug", $(this).val(), {
			expires : 60,
			path : "/"
		});
		window.scrollTo(0, 0);
	...
	if ($(".block.list .screens").length) {
		var unslider = $(".block.list .screens").unslider(
				{
					delay : false,
					complete : function(el) { 
						var unslider = el.data("unslider");
						var hovered = $(".howitworks li.active").index();
						if (unslider.i != hovered && hovered > -1) {
							setTimeout(function() {
								unslider.to(hovered)
							}, 100)
						}
						if (unslider.i == 0) {
							$(".howitworks li").first().addClass("hover")
									.siblings().removeClass("hover")
						}
					}
				}).data("unslider");
		$(".block.list .howitworks li").mouseover(function() {
			$(this).addClass("hover").siblings().removeClass("hover");
			var new_index = $(this).index();
			if (unslider.i != new_index) {
				unslider.to(new_index)
			}
		}) 
	}
	var close_popups = function() {
		$f($("#video iframe")[0]).api("pause");
		$("#overlay").hide("fade");
		$(".popup").hide("fade")
	};
	$(".video-button, .video-link").click(function() {
		$("#overlay, #video").show("fade")
	});
	...
	var citations = $(".press.block .citation");
	var random = Math.round(Math.random() * (citations.length - 1));
	citations.eq(random).show();
	var lights = [];
	var light_count = 7;
	jQuery.fx.interval = 39; 
	var defocus = function(light) {
		var width = $(".block.top").width() - $(light).width() - 20;
		var height = $(".block.top").height() - $(light).height();
		var speed = 1500 + Math.round(Math.random() * 2300);
		$(light).css({
			opacity : .1 + Math.random() * .3,
			top : Math.random() * height,
			left : Math.random() * width,
			height : 30 + Math.random() * 34
		}).animate({
			opacity : "toggle"
		}, speed).animate({
			opacity : "toggle"
	...
	$(".list.block .howitworks h3").waypoint(function() {
		var elements = $(this).closest(".howitworks").find("ul li");
		var params = {
			opacity : 1
		};
		$(elements[0]).addClass("hover");
		$(elements[0]).animate(params, 2200, function() {
			unslider.to(1);
			setTimeout(function() {
				$(elements[0]).removeClass("hover")
			}, 500);
			$(elements[1]).addClass("hover");
			$(elements[1]).animate(params, 2200, function() {
				unslider.to(2);
				setTimeout(function() {
					$(elements[1]).removeClass("hover")
				}, 500);
				$(elements[2]).addClass("hover");
				$(elements[2]).animate(params, 2200, function() {
					unslider.to(0);
					setTimeout(function() {
						$(elements[2]).removeClass("hover")
					}, 500)
				})
			})
		})
	...
	partners = [ "beyonce", "elliegoulding", "sofarsounds", "theO2", "hotchip",
			"sadlerswells", "chessboxing", "roundhouse", "kanyewest",
			"royalcourt", "futurecinema", "surreycricket", "barbican",
			"comedystore", "brunelmuseum", "royalalberthall", "udderbelly",
			"southbankcentre", "florenceandthemachine", "simianmobiledisco" ];
	shuffleArray(partners);
	$(".block.press .press-links").waypoint(function() {
		$("img", this).animate({
			opacity : .8
		}, 1200)
	}, {
